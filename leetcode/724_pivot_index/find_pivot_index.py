...
# We need to quickly compute the sum of values to the left and the right of every index. Let's say we knew S as the sum of the numbers, and we are at index i. 
...
#Time Complexity:
# O(N) as we need to iterate the entire array once to calculate the sum and once to compute the prefix sum.
#Space Complexity:
# O(1) as no extra space utilized. 


#Tests:
# python test_find_pivot_index.py